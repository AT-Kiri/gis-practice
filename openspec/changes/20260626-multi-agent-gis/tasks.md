...
### Task 14: 算法层 - Pareto 多目标优化 + ACO 蚁群路径分配
- **优先级**: P2
- **依赖**: Task 11
- **状态**: done
- **描述**: Pareto 多目标资源优选 + ACO 蚁群多车路径分配，用确定性算法约束 LLM 不可靠的资源调度推理
- **实现说明**: 两个算法都做了完整实现（非 mock），作为 LangChain Tool 被 RouteAgent 调用。第一性原理分析确认两个算法与应急场景高度契合
- **子任务**:
  - [x] Pareto 多目标资源优选工具（非支配排序 + 综合评分 + 推荐理由）
  - [x] ACO 蚁群多车路径分配工具（信息素正反馈 + 精英策略 + Haversine 距离）
  - [x] 工具输出统一契约（ToolResult + GeoJSON 可视化）
  - [x] 注册到 RouteAgent 子 Agent（工具列表 + 系统提示词规则 8/9/10）
  - [x] 单元测试验证（Pareto 前沿正确性 + ACO 分配完备性 + 边界场景）
  - [x] 前端 ToolCallCard 工具名中文映射更新
