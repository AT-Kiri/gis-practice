 ...
 * <p>
 * RoadNet@Changchun 使用的 PCS_NON_EARTH 平面坐标系（单位：米）无标准 EPSG 编码，
 * 因此通过线性插值在 WGS84 地理范围与平面坐标范围之间建立一一映射关系。
 */
public class CoordConverter {
    ...
    // ---- RoadNet@Changchun 数据集平面坐标范围（单位：米） ----
    // 该范围由数据集实际 bounds 确定：x=[47, 8958], y=[-7669, -55]
    ...
    /**
     * 将 WGS84 经纬度转换为 Changchun 平面坐标（用于构建 iServer 网络分析请求）
     * @param lng 经度
     * @param lat 纬度
     * @return [x, y] 平面坐标（米）
     */
    ...
    /**
     * 将 Changchun 平面坐标转换为 WGS84 经纬度（用于将 iServer 分析结果还原到地图显示）
     * @param x 平面 X 坐标（米）
     * @param y 平面 Y 坐标（米）
     * @return [lng, lat] 经纬度
     */
    ...
    /**
     * 批量转换 WGS84 坐标点为 Changchun 平面坐标
     * @param points WGS84 坐标数组，每项为 [lng, lat]
     * @return 平面坐标数组，每项为 [x, y]
     */
