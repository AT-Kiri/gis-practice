...
7. 复杂应急任务的推荐 6 步流程（适用于地震/火灾/洪水等灾害评估+救援场景）：
   【双缓冲区半径表 - 必须严格按此表传参，禁止 LLM 自定半径】
   地震：inner_distance=3000, outer_distance=8000
   火灾：inner_distance=1000, outer_distance=3000
   洪水：inner_distance=2000, outer_distance=5000
   步骤1 - search: 定位受灾地点（region=jingjin），获取名称和坐标。多个匹配时取名字完全匹配的。受灾地点 Point 会自动标记为受灾中心，作为步骤5路径规划终点
   步骤2 - analysis: 调用 dual_buffer_analysis(center=受灾点坐标, inner_distance=小半径, outer_distance=大半径)，按灾害类型查半径表。返回 inner_geometry_brief(受灾圈) 和 outer_geometry_brief(支援圈)
   步骤3 - search: 用受灾圈 Polygon 做空间查询。spatial_query(geometry=inner_geometry_brief, feature_type=point) 查受灾范围内行政/人口点要素。查到 0 条属正常现象（数据源是行政点，小缓冲区可能无匹配），禁止 mock 受灾点，禁止对同一参数重复调用同一工具
   步骤4 - search: 在支援资源环带生成模拟资源。mock_nearby_resources(center=受灾点坐标, inner_radius=小半径, outer_radius=大半径, resource_type=hospital, count=3) 在大-小环带生成3个模拟医院。必须告知用户是模拟数据
   步骤5 - route: 对步骤4的3个模拟资源，取前2条分别规划到【步骤1受灾地点Point】的路径（终点严禁用缓冲区质心，必须用受灾地点 Point）。京津冀用 online_route_planning，长春用 shortest_path
   步骤6 - knowledge: 检索该类灾害的应急救援知识库内容（处置流程、预案等）
   【最终方案汇总由 Coordinator 自动完成，无需规划单独步骤】前6步结果会自动汇总成应急救援方案（含灾情评估、推荐救援资源、行进路线、处置建议）
8. 路径规划规则：
   - 京津冀地区（如朝阳区、丰台区等）：使用 online_route_planning，起点是支援资源点，终点是【步骤1受灾地点Point】（严禁用缓冲区质心作终点）
...
2. analysis: "调用 dual_buffer_analysis(center=朝阳区坐标, inner_distance=3000, outer_distance=8000) 生成双缓冲区（直接复用步骤1注入的坐标）"
3. search: "用受灾圈 Polygon 查受灾范围内点要素。spatial_query(geometry=inner_geometry_brief, feature_type=point)。查到0条属正常，禁止mock"
