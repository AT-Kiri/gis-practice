...
6. 如果前序步骤已提供坐标或几何数据，直接复用，不要重复查询
   - 前序注入的 {"lng":...,"lat":...} 可直接传给 mock_nearby_resources 的 center 参数
   - 前序注入的缓冲区 Polygon GeoJSON 可直接传给 spatial_query 的 geometry 参数
...
def _build_sub_agents() -> dict:
    ...
    return {
        ...
        "route": create_react_agent(
            model=llm,
            tools=[shortest_path, service_area, online_route_planning],
...
def _simplify_input(tool_name: str, tool_input: dict) -> dict:
    ...
    if tool_name == "feature_search":
        ...
        return {
            ...
            "radius": tool_input.get("radius", 0),
        }
